...
| --- | --- | --- |
| **0 — Foundation** ✅ *(in progress)* | `/docs`, ADR-001…005, repo scaffold plan. | A new engineer understands the system from `docs/` before opening code. |
| **1 — App foundation** | Auth (magic-link), org onboarding, users, roles, permissions, app shell + navigation, DB, migrations, tenant isolation (repo + RLS), audit log (hash-chained). | A user creates an org, invites users, logs in, works in a secure tenant-isolated workspace; every consequential action is audit-logged. |
| **2 — Supplier intelligence** | Supplier directory, create/detail, relationships, invitation, supplier portal foundation, Supplier Passport, supplier evidence. | An enterprise invites a supplier and collects sustainability information. |
| **3 — Evidence infrastructure** | Document upload/storage/metadata, evidence records + lifecycle, evidence viewer, versioning, linking, audit history. | Any sustainability datapoint can be linked to evidence. |
